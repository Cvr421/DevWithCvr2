...
export default function Hero({pageInfo}: Props) {
    const [text , count] = useTypewriter({
        words: [
            ...
            `${pageInfo?.name} Here , This is Development Platform `,
  ...
  return (
    ...
    <div className="z-20">
        <h2 className="text-xs md:text-sm uppercase text-[#F7AB0A] pb-2 tracking-[8px]">{pageInfo?.role}</h2>
      <h1 className="text-5sl lg:text-2xl font-semibold px-10">
       <span className="mr-3">{text}</span>
        <Cursor cursorColor="#F7AB0A"/>
       </h1>

       <div className=" pt-10  md:pt-5 flex flex-row justify-between">
        <Link href="#About">
